...
def getControls(topNode: "controls", selection=True):
    ...
    if not selectedNodes:
        m.warning("please select one node in the scene")
    return []
    ...
    allChildren = mc.listRelatives(selectedNodes, allDescendents=True, type="transform") or []
...
def applyPoseData(poseData, selectedControls=None, excludeRootMainControls=True, keyPoseControls=False):
    selectedNodes = mc.ls(selection=True)
    if not selectedNodes:
        mc.warning("please select one node in the scene")
        return
